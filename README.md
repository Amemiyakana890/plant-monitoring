...
※2 光センサーは当初M5Stack用光センサユニット(U021)を採用予定でしたが、購入しようとした時点で品切れとなっており入手できませんでした。もともと拡張ベース経由でのI2C/ADCピン競合・ピン不足の懸念も抱えていたため、それらも踏まえて入手可能だったBH1750(ENV IIIとは別のI2Cバス経由)へ変更しました。詳細は[設計書 8章](docs/system-design.md#8-デバイス設計)を参照してください。
...
- センサー:M5Stack用温湿度気圧センサユニット Ver.3（ENV Ⅲ）、M5Stack用 土壌水分センサユニット(Unit Earth、U019、砂ポケット運用)、光センサーユニット BH1750
