...
- 電源:小型モバイルバッテリー(今後購入予定)
- 開発モジュール:ATOM Matrix
- 拡張:ATOM PortABC拡張ベース
- センサー:M5Stack用温湿度気圧センサユニット Ver.3（ENV Ⅲ）、M5Stack用土壌水分センサユニット [U019]、M5Stack用光センサユニット (今後購入予定)[U021]
- カラーバリエーション:ミルクホワイト / セージグリーン / グレージュ(3Dプリンターで型番作成を行いたいです)
