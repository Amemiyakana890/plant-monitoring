...
| 4 | 土壌水分センサー(DFRobot Gravity 防水静電容量式 SEN0308)※ | アナログ出力(ADC接続) |
| 5 | 光センサーユニット(予定) | (GPIO接続) |
| 6 | 小型モバイルバッテリー | USB-C出力 |
| 7 | その他受動部品 | 抵抗・コンデンサ等 |

※ 当初はM5Stack用土壌水分センサユニット(U019、抵抗式)を使用していたが、接続不良の切り分けの結果SEN0308(静電容量式)へ変更した。プランターに挿しっぱなしにする本プロジェクトの用途では、電極が防錆・防水加工されていないU019はPOC(検証)用途止まりであった点も変更理由の一つ。詳細は[デバイス検証ログ](device-test-log.md)を参照。

### 8-2. 接続構成(ブロック図)

```
ATOM Matrix ──┐
    |         └─▶ ATOM PortABC拡張ベース ──▶ PortA:ENV Ⅲ(I2C接続)
    |                                   ├─▶ PortB:土壌水分センサー SEN0308(アナログ/ADC接続)
...
| 通信方式 | Wi-Fi 2.4GHz |
| 電源 | モバイルバッテリー |
| センサー | 温度・湿度・気圧(ENV Ⅲ)、土壌水分(SEN0308)、照度 |
| 動作温度 | 0〜50℃ |
