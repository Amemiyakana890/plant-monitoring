...
| 3 | ENV Ⅲ | 温度・湿度・気圧センサー(I2C接続) |
| 4 | 土壌水分センサー(M5Stack用 Unit Earth、U019、抵抗式)※ | アナログ出力(ADC接続、ATOM Matrix本体Groveポート直挿し・GPIO32)。センサー周辺に砂ポケットを設けて運用 |
| 5 | 光センサーユニット(予定) | (GPIO接続) |
| 6 | 小型モバイルバッテリー | USB-C出力 |
| 7 | その他受動部品 | 抵抗・コンデンサ等 |

※ 土壌水分センサーはU019(抵抗式)→DFRobot Gravity 防水静電容量式土壌水分センサー V2.0(SEN0308、静電容量式)→U019(砂運用)という経緯を経て、最終的にU019に決定した。当初U019で見られた異常値(0固定、4095張り付き、数十ms単位の暴れなど)は培養土との相性(電極接触ムラ)が原因と判明し、砂状の媒体を用いることで安定動作を確認した。なお実装時、ATOM PortABC拡張ベースのPort B経由ではRAW値がADC最大値付近に張り付く現象が再発したため、配線をATOM Matrix本体のGroveポート直挿し(GPIO32)に変更し、安定動作を確認した(拡張ベースPort Bは不採用)。電極の防錆・防水加工が無くM5公式もPOC(検証)用途としている点は変更されないため、長期の腐食・劣化リスクは残存する制約として許容した上での採用である。詳細は[デバイス検証ログ](device-test-log.md)を参照。

### 8-2. 接続構成(ブロック図)

```
ATOM Matrix ──┬─▶ ATOM PortABC拡張ベース ──▶ PortA:ENV Ⅲ(I2C接続)
              │                          └─▶ PortC:(※光センサーはUART接続できないのでPortCは何も入れない)
              └─▶ 本体Groveポート直挿し:土壌水分センサー U019(GPIO32、ADC接続、砂ポケット運用)
(予定)
モバイルバッテリー ─▶ USB-C ─▶ ESP32
```

> 土壌水分センサーは当初ATOM PortABC拡張ベースのPort B経由(GPIO33)で接続していたが、
> RAW値がADC最大値付近に張り付く現象が発生したため、ATOM Matrix本体のGroveポート
> 直挿し(GPIO32)に変更した(詳細は[デバイス検証ログ 2026-08-04](device-test-log.md)を参照)。
...
| 通信方式 | Wi-Fi 2.4GHz |
| 電源 | モバイルバッテリー |
| センサー | 温度・湿度・気圧(ENV Ⅲ)、土壌水分(U019、本体Grove直挿し・砂ポケット運用)、照度 |
| 動作温度 | 0〜50℃ |
